...
**Status:** Open. Baseline is the coaxial X8. This document keeps the credible alternatives sized on the same assumptions, so the team can pivot quickly if tests or requirements change. The decision is made at the Preliminary Design Review (PDR).
...
Every layout carries the same design point and fixed equipment so the comparison is fair.
...
- Fixed items in every layout: landing gear 3.0 kg, HV distribution 1.0 kg, avionics and LV power 4.0 kg, seat and controls 7.3 kg, ballistic parachute 9.5 kg.
...
- **Airframe and mount masses for the alternatives are engineering estimates** scaled from the baseline, not designs.

---

## Summary

| | **Coaxial X8** (baseline) | **Flat octo** | **Coaxial X12** | **Flat hexa** |
| --- | --- | --- | --- | --- |
...
**Pivot to the coaxial X12 if the motor-out thermal test on the X13 G2 fails** (baseline open item 1). The X12 spreads the load across more, lighter-loaded units, which directly fixes that failure mode. It only works if the parachute exclusion is confirmed or mass comes out elsewhere.

---

## Flat hexacopter (6 arms)
...
1. **Coaxial loss.** Run one X13 G2 alone, then stacked in a coaxial pair on the thrust stand. If the pair makes much less than 1.7 × one rotor's thrust, the flat octo gains ground.
2. **Motor-out thermal.** Hold one X13 G2 at 40 to 45 kgf for 120 s. Failing this moves the baseline toward the coaxial X12.
3. **Parachute exclusion.** FAA's reading decides whether the coaxial X8 and X12 have real margin.
4. **Folding arm design.** Mass and stiffness of a real hinge sets the arm penalty for the flat octo, X12, and flat hexa.
...
- Span: rotor positions placed on a circle with adjacent spacing 1.1 D; span = 2 × circle radius + D. For the coaxial X8 (4 positions) this equals the diagonal span.
- Endurance: 6.5 kWh × pack scale × 80% usable ÷ hover power.
- One motor out: (n − 1) × rotor thrust × coaxial factor ÷ gross weight. One pack out removes 2 motors (3 for the X12).
- Airframe estimates: the flat octo adds about 6 kg for 8 longer folding arms; the X12 adds about 0.7 kg for 6 short arms plus 1 kg of stacked mounts; the flat hexa adds about 1.7 kg for 6 longer arms. All replaced by CAD and FEA mass once designs exist.
